...
Assigned output directory: ideas/actioner-002
...
===== ideas/actioner-002/action_state.md =====
# Action state (mechanically collected -- facts, not judgments)

## Consensus recommendations for live ideas
- **idea-002** [PAUSED]: **PAUSE** — the single most important thing for the human to inspect is the original annotation protocol, specifically whether checklist scorers could see the paired clinical photograph or other clinical information. Until that is resolved, the surviving experiment has irreducibly ambiguous value; if exposure to both images is confirmed, stop at a short documentation finding rather than train an image model.

- **idea-004** [ACTIVE]: **REVISE.** The debate converged on a defensible design, but the current idea card still contains claims and scores that the debate explicitly withdrew. Before deciding whether to advance to a feasibility memo, the human should look first at the direct Stage 0 metadata counts—especially the number and parameter makeup of geometry-matched same-acquisition pairs—because that single inspection determines whether the stronger reconstruction-content study exists or only the narrower composite pipeline audit remains.

- **idea-006** [PAUSED]: **PAUSE.** Before deciding whether the pause is reversible, the human should inspect the official CT-CLIP training data loader and augmentation configuration for large-region masking or cutout with a matching fill value. Absence would make the original intervention indefensible for this checkpoint; presence would justify distributional validation, not automatic advancement.

- **idea-007** [ACTIVE]: **REVISE.** Update the idea card to the converged state-level claim and corrected scores, then require Stage 0 before a probe contract. The single most important thing for the human to inspect is the prespecified DICOM-to-final-tensor comparability gate: whether enough inhale/exhale pairs truly retain matched reconstruction, coordinates, physical scale, and thoracic coverage through the complete pinned CT-CLIP preprocessing pipeline.

- **idea-008** [ACTIVE]: **REVISE.** The debate produced a coherent conditional design, but the current `idea_card.json` still describes the superseded reconstruction arm, rung-3 observational logic, `INSPECTED_TRUE` keystone, and obsolete scores. Before deciding whether to advance, the human should look most closely at whether the proposed tissue-for-tissue edit can be validated as in-distribution with a prespecified sham-effect tolerance; that is now the single fact separating a model-use study from an association-only study.

- **idea-011** [PAUSED]: **PAUSE.** The debate converged after a real, persistent objection; this was not a one-round rubber stamp. Before deciding whether to reopen, the human should look first for the missing identification instrument: a confirmed human chest CT resource with retained spectral base-material or dual-kV raw data and linkable age that can provide a measured, post-preprocessing-matched mineralized-to-soft-tissue contrast. Without that—or another genuinely matched real-tissue control—the current experiment cannot distinguish native use of costal cartilage mineralization from response to the deletion operation, regardless of improvements to masks, models, or supervision audits.

- **idea-012** [PAUSED]: **PAUSE.** Before deciding otherwise, the human should inspect whether the MD.ai-derived scan-level exclusion membership has become available—and is joinable to a frozen obtainable Sybil evaluation split—because without it the study cannot test the specific residual that defines Idea 012.

- **idea-013** [SHORTLISTED]: **REVISE.** Rewrite the idea card around the reduced rung-1 calibration and reconstruction-sensitivity audit, with localisation explicitly excluded and the keystone set to `NOT_INSPECTED`. The single most important thing for the human to inspect before deciding is whether a validated, annotation-free coronary target localiser for nongated noncontrast CT actually exists and runs on CT-RATE; that fact determines whether the high-value localisation question has a credible spin-off or whether idea 013 should remain only a modest robustness audit.

- **idea-014** [PAUSED]: **PAUSE.** The revised question remains scientifically coherent, but the rate-limiting model asset has not been reproduced, and the final edit-validity amendment has not been tested or answered by the critic. The single most important thing for the human to inspect before deciding is whether the released Pierson pipeline can, with available OAI access and the allowed compute, produce at least two independent models that pass preregistered better-than-KLG and disparity-signature gates on a frozen split. Without that, there is no model family to decode; editor validity is the next gate only after reproduction succeeds. ```json {"verdict": "PAUSE", "unblock": "Demonstrate at least two independently trained Pierson-recipe models that pass preregistered pain-performance and disparity reproduction gates on a frozen OAI split within the available access and compute constraints."} ```

- **idea-017** [SHORTLISTED]: **REVISE.** Rewrite `idea_card.json` to implement the converged Stage 0-only design before any feasibility memo or probe contract. The single most important thing for the human to inspect is whether the joint-support gate can be given a prespecified, adequately powered minimum-support criterion: if continuous tracheal index cannot be separated from sex, emphysema, lung volume, and reconstruction in a recoverable held-out cohort, idea 017 dies like idea 009 regardless of the attractiveness of its mechanism. ```json {"verdict":"REVISE","unblock":"Rewrite the idea card as a four-gate Stage 0-only study with no erasure or use claim, then prespecify and inspect adequate independent tracheal-index support in a recoverable Sybil-held-out or external cohort."} ```

- **idea-019** [SHORTLISTED]: **REVISE.** The debate produced a coherent conditional rung-1 feasibility/use study, but the existing card is materially stale and the program's required physician-legible endpoint remains outside the evidence. Before deciding whether revision-in-place is permissible, the human should look most closely at the claim-identity boundary: whether replacing “the model uses honeycombing” with “the model uses a prespecified subpleural cyst-network topology index” is rung honesty within the same candidate or a new deliverable sentence that must be registered as a successor. ```json {"verdict": "REVISE", "unblock": "Human resolves the claim-identity boundary, then the card is rewritten to the agreed index-level, K1-gated, G2a/G2b-conditional rung-1 design (or registered as a successor if the deliverable sentence is judged changed)."} ```

## Paused ideas and their unblock notes
- idea-002: Annotation provenance undocumented: unknown whether checklist annotators saw the paired clinical photograph. A positive result would have two materially different explanations. Unblocked only by author correspondence.
- idea-006: Patient-deletion is an extreme OOD intervention; neither direction identifies exterior reliance during intact inference. Wrong-keystone error, third occurrence: verified that exterior voxels survive preprocessing (true) when the inference required that a body-excluded volume is in-distribution (unve
- idea-011: needs spectral/dual-kV matched mineralization control
- idea-012: await MD.ai cohort membership release
- idea-014: debate: Demonstrate at least two independently trained Pierson-recipe models that pass preregistered pain-performance and disparity reproduction gates on a frozen OAI split within the available access and compute constraints.

## Banked fiction near-misses (unadopted)
- (none)

## Backlog (19 candidates, best first)
- 006-C4 [NOVEL_UNVERIFIED, audited 2026-08-10] Merlin predicts osteoporosis - ask whether it reads the density of the bone or the shape o
- 009-C8 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The glioblastoma prognosticator may be reading the invasion front's roughness
- 009-C6 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The CT spirometer may be reading the diaphragm as a pressure-loaded membrane
- 008-C4 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The emphysema call may read the shape of the holes, not just how many
- 008-C5 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The lung-cancer model may read the aorta as an ageing clock
- 007-C6 [NOVEL_UNVERIFIED, audited 2026-08-10] The effusion model may be reading whether pleural fluid still obeys gravity
- 009-C9 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The arterial-calcification score may be reading inspiratory depth
- 009-C4 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The risk model may be reading the breast's lines of force
- 007-C8 [NOVEL_UNVERIFIED, audited 2026-08-10] The PE model may be reading how completely blood and contrast have mixed
- 009-C1 [NO_DUPLICATE_FOUND_LIMITED_SEARCH, audited 2026-08-11] The CT spirometer may be measuring remodeled airway walls
- 007-C3 [NOVEL_UNVERIFIED, audited 2026-08-10] Merlin may be reading fatty kidney rather than kidney shape
- 006-C5 [NOVEL_UNVERIFIED, audited 2026-08-10] An airway and its artery run together and should taper together - ask whether the model re

## Latest librarian pass (librarian-001)
...
## Previous brief (verbatim -- ground persistence and resolution claims here)

# Action brief

## Decisions waiting on the human

- **idea-015 — decide candidate identity before any further work.** Should the narrowed BAC-response experiment remain a revision of Idea 015, or be registered as a new candidate because the original vascular-age interpretation has been removed? Revision preserves continuity; re-registration keeps the record semantically clean. **Recommendation: register a new candidate**, because the deliverable claim and prohibited conclusions have materially changed. This governance question has persisted across multiple briefs; either choice must retain the agreed BAC-specific design and require Stage 0 verification of an obtainable Mirai-compatible cohort plus BAC-mask registration to Mirai's exact input tensor.

- **idea-004 — accept revision and move to a feasibility memo, or narrow it further.** Advancing preserves the stronger reconstruction-content study only if the already collected Stage 0 pair counts and acquisition-parameter makeup support it; otherwise the study becomes a composite pipeline audit. **Recommendation: revise and advance conditionally**, using the direct 425-pair metadata audit as the scope gate. This has remained active across multiple briefs despite completed Stage 0 work.

- **idea-007 — accept the rung-1 respiratory-state claim or retire it.** Revision keeps only sensitivity to respiratory state and requires the full DICOM-to-final-tensor comparability gate; rejection avoids work if that narrower claim is not portfolio-worthy. **Recommendation: revise the card and run Stage 0**, because the paired within-session design is unusually strong, but no probe contract should follow unless reconstruction, coordinates, scale, and thoracic coverage survive preprocessing. This has waited across multiple briefs.

- **idea-008 — accept the conditional tissue-for-tissue edit design or pause it.** Revision permits a model-use study only if the edit passes a prespecified in-distribution sham-effect tolerance; pause treats the current evidence as association-only. **Recommendation: revise, then pause at the edit-validity gate**, because passing that gate is the difference between identifying emphysema use and merely inducing an edit response. This has waited across multiple briefs.

- **idea-013 — keep a modest robustness audit, or wait for a credible localisation study.** Revision yields a rung-1 calibration and reconstruction-sensitivity audit with localisation explicitly excluded; pausing avoids spending effort on a claim of limited interpretive value while checking for a validated annotation-free coronary localiser. **Recommendation: revise the card but do not prioritize execution**, unless the human values the reduced audit independently of localisation. This has waited across multiple briefs.

- **idea-014 — continue the pause or fund model reproduction.** Continuing the pause acknowledges there is no reproducible model family to decode; funding reproduction requires at least two independent Pierson-recipe models to pass frozen-split performance and disparity-signature gates before editor validity is considered. **Recommendation: keep paused**, because the unblock is substantive compute work rather than a documentation decision. This has waited across multiple briefs.

- **idea-003 and idea-009 — ratify rejection.** Keeping either active would preserve, respectively, a BI-RADS intervention question unsupported by public assets and a vascular-mechanism claim not identified by its proposed association/deletion tests. Rejection closes them while retaining explicit revisit conditions. **Recommendation: reject both**, because further amendments would answer different questions. Both have remained marked active despite consensus rejection.

- **idea-002, idea-006, idea-011, and idea-012 — ratify their pauses.** Advancing now would ignore, respectively, unresolved annotation exposure, an unvalidated OOD intervention, a missing matched mineralization control, and unavailable scan-
