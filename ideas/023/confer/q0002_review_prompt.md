...
Signs were (+, −, +); the two zero-excluding intervals point in opposite directions, so the conjunction fails on direction [cite: interpretation.md | Demonstrates item 2] [cite: summary.json | g_label_passed=false, status=NEGATIVE_PATTERN]. Crucially, all three bands had 99 contributing patients against a floor of 20, and CI widths (0.0652, 0.0479, 0.0386) beat the 0.15 bound, so this is the contract's decisive negative — not a power or support failure [cite: per_stratum_summary.csv | stratum=1,2,3 | patients, ci_width] [cite: resolved_config.json | minimum_contributing_patients_per_stratum=20, maximum_primary_ci_width=0.15] [cite: decision.md | Layer A — Finding]. A decisive negative on directional *consistency* is not a null on *presence of information*: the middle and highest bands each carry a precise, real association; they simply disagree in sign [cite: interpretation.md | Suggests item 1].

**Why "CBV doesn't matter biologically" is doubly unsupported.** First, the identity census shows CBV and MTT are effectively one degree of freedom at fixed CBF in these maps — median absolute centered residual 0.0078 / 0.0036 / 0.0078 across bands, far below the 0.10 limit [cite: identity_residual_summary.csv | stratum=1,2,3 | median_absolute_centered_residual] [cite: interpretation.md | Demonstrates item 3]. So the study cannot even separate a "CBV channel" from an "MTT channel"; channel attribution is explicitly prohibited [cite: interpretation.md | Out-of-scope warnings; Does not establish]. Second, a documented confound sits under the negative: the label-blind NCCT audit's cited example cases show large Q1-vs-Q4 attenuation imbalance (e.g. sub-stroke0092 band 1: Q1 median 3.0 HU vs Q4 23.0 HU), meaning the compared low-CBV cells can be frankly hypodense already-injured tissue rather than a matched hemodynamic state [cite: bin_tissue_audit.csv | case_id=sub-stroke0092, stratum=1, style_group=Q1_low_CBV/Q4_high_CBV | median_hu] [cite: interpretation.md | Suggests item 3, INFERENCE]. Cohort prevalence of that imbalance was not quantified, so it is a bounded design concern, not a proven cause [cite: interpretation.md | Suggests item 3; Does not establish]. The recorded consequence is a *tissue-normalized* successor design — i.e., a reason to re-measure the perfusion coordinate more carefully, not to discard it [cite: interpretation.md | Next decision item 3] [cite: decision.md | Verdict].
...
**What the evidence does support instead.** The negative pauses idea-023 and blocks the model-use probe under this operationalization [cite: decision.md | Verdict] [cite: CARD.md | ledger status: PAUSED]. It also yields positive, reusable signal for *keeping and better-measuring* perfusion: the identity residual validates the one-degree-of-freedom framing, two bands show precise opposite-signed associations, and the per-case HU audit gives direct design input for a tissue-normalized matched-flow successor (parent idea-023) [cite: interpretation.md | Positive and negative findings, Positive; Next decision item 3] [cite: decision.md | Verdict]. That is the recorded forward path — a successor that normalizes tissue and revisits the perfusion coordinate — the opposite of abandoning perfusion.
...
2. **Prefer the already-recorded successor over abandonment.** Register the tissue-normalized matched-flow successor under parent idea-023 through the normal pipeline, using the band-2/band-3 sign reversal and the `bin_tissue_audit.csv` rows as its empirical starting points, before concluding anything about the value of the perfusion coordinate [cite: interpretation.md | Next decision item 3] [cite: decision.md | Verdict].
...
4. **Successor-question sketch:** "Under a tissue-normalized reference at matched CBF, does the joint CBV/MTT coordinate carry a directionally consistent final-infarct association across within-patient flow bands in the icobrain cva treated cohort?" — this keeps the perfusion coordinate central rather than discarding it [cite: interpretation.md | Next decision item 3; Suggests item 1].
