...
class LifecycleTests(unittest.TestCase):
    def test_full_synthetic_command_path(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(c,'require_review'),patch.object(c,'load_validator',return_value=SimpleNamespace(verify=lambda *a:{'status':'SYNTHETIC_ADAPTER'})):
                ...
                self.assertEqual(len(git('log','--format=%H').splitlines()),6)
