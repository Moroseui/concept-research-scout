...
def install(source_root, source, registration, consent, decision):
    ...
    if (
        ...
        or grant.get("source") != source
        or grant.get("scope") != SCOPE
        or grant.get("capability") != "registered-drive-evidence-v1"
    ):
    ...
    if set(json.loads(token).get("scopes", [])) != {SCOPE}:
        raise ValueError("NARROW_TOKEN_REQUIRED")
    paths = [
        Path("/etc/research-system/drive"),
        Path("/var/lib/research-system/drive-evidence"),
        Path("/var/lib/research-system/drive-upload-spool"),
        Path("/opt/research-system/drive"),
        Path("/etc/systemd/system/research-system-drive.socket"),
        Path("/etc/systemd/system/research-system-drive.service"),
    ]
    if any(p.exists() or p.is_symlink() for p in paths):
        raise ValueError("EXISTING_INSTALLATION_RECONCILE_NO_RETRY")
    if not Path("/opt/research-system/drive-venv/bin/python").is_file():
        raise ValueError("PINNED_CLIENT_ENVIRONMENT_REQUIRED")
    for p in [root, *root.parents]:
        if p.stat().st_uid != 0 or p.stat().st_mode & 0o022:
            raise ValueError("ADMIN_OWNED_SOURCE_REQUIRED")
    ...
    try:
        ...
        identity = pwd.getpwnam("research-drive")
    if identity.pw_uid == 0 or identity.pw_shell != "/usr/sbin/nologin":
        raise ValueError("DEDICATED_NONROOT_IDENTITY_REQUIRED")
    admin = paths[0]
    admin.mkdir(mode=0o700)
    private_write(
        admin / "installation-intent.json",
        {
            "source": source,
            "grant_sha256": hashlib.sha256(protected_read(decision)).hexdigest(),
        },
    )
    config["installed_source"] = source
    private_write(admin / "config.json", config)
    with (admin / "oauth.json").open("xb") as out:
    ...
    for name in ("research-system-drive.socket", "research-system-drive.service"):
        ...
        with (Path("/etc/systemd/system") / name).open("xb") as out:
            out.write(raw)
        (Path("/etc/systemd/system") / name).chmod(0o644)
    subprocess.run(["systemctl", "daemon-reload"], check=True)
    private_write(
        admin / "installation-receipt.json",
        {
...
if __name__ == "__main__":
    ...
    for name in ("source-root", "source", "registration", "consent", "decision"):
        p.add_argument("--" + name, required=True)
    a = p.parse_args()
    install(a.source_root, a.source, a.registration, a.consent, a.decision)
