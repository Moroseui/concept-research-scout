...
def read_run_bundle(spool):
    """One shared bounded reader for staging and upload, with no console disclosure."""
    if spool.is_symlink() or not spool.is_dir():
        raise ValueError("REGISTERED_RUN_BUNDLE_REQUIRED")
    ...
    for name in files:
        if (
            "/" in name
            or not re.fullmatch("[A-Za-z0-9_.-]{1,100}", name)
            or Path(name).suffix
            not in (".log", ".json", ".ipynb", ".md", ".csv", ".txt")
        ):
            raise ValueError("BOUNDED_RUN_ARTIFACT_NAMES_REQUIRED")
        p = spool / name
        if p.stat().st_size > LIMIT:
            raise ValueError("DRIVE_TRANSFER_SIZE_BOUND")
        if sum(map(len, payloads.values())) + p.stat().st_size > LIMIT:
            raise ValueError("DRIVE_TRANSFER_SIZE_BOUND")
        fd = os.open(p, os.O_RDONLY | os.O_NOFOLLOW)
        with os.fdopen(fd, "rb") as handle:
            payloads[name] = handle.read(LIMIT + 1)
        if len(payloads[name]) > LIMIT:
            raise ValueError("DRIVE_TRANSFER_SIZE_BOUND")
        from orchestrator.git_publication import SECRET

        if SECRET.search(payloads[name]) or re.search(
            rb"(?:ya29\.[A-Za-z0-9_-]{20,}|1//[A-Za-z0-9_-]{20,})", payloads[name]
        ):
            raise ValueError("CREDENTIAL_IN_UPLOAD_REFUSED")
    if sum(map(len, payloads.values())) > LIMIT:
        raise ValueError("DRIVE_TRANSFER_SIZE_BOUND")
    if (
        any(digest(payloads[n]) != h for n, h in files.items())
        or inventory(spool) != files
    ):
...
class GoogleDrive:
    ...
    def __init__(self, credentials):
        ...
        if set(info.get("scopes", [])) != {SCOPE}:
            raise ValueError("NARROW_STORED_SCOPE_REQUIRED")
    ...
    def metadata(self, file_id):
        return (
            self.api.files().get(fileId=file_id, fields=FIELDS).execute(num_retries=0)
        )
    ...
    def private_folder(self, file_id):
        value = (
            ...
            .get(
                fileId=file_id,
                fields="id,mimeType,trashed,ownedByMe,permissions(type,role)",
            )
        ...
        if (
            ...
            or not value.get("ownedByMe")
            or not value.get("permissions")
            or any(
                p.get("role") != "owner" or p.get("type") != "user"
    ...
    def allocate_ids(self, count):
        return (
            self.api.files()
            .generateIds(count=count, space="drive", type="files")
            .execute(num_retries=0)["ids"]
        )
    ...
    def create(self, file_id, name, parent, data=None):
        ...
        if data is not None:
            args["media_body"] = MediaIoBaseUpload(
...
class DriveEvidence:
    ...
    def handle(self, request, peer_uid):
        ...
        if (
            ...
            or set(request) != {"operation", "body"}
            or not isinstance(request["body"], dict)
            or set(request["body"]) != {"alias", "request_id"}
        ):
        ...
        if request["operation"] == "status" and alias == "run-artifacts":
            return self.upload_status(self.root / ("upload-" + request_id))
        ...
        if folder.exists():
            if (folder / "intent.json").exists() and json.loads(
                (folder / "intent.json").read_text()
            )["operation"] != request["operation"]:
                raise ValueError("DRIVE_REQUEST_ID_CONFLICT")
            return self.status(folder, alias)
        if (
            request["operation"] == "collect"
            and entry["access"] != "small-evidence-read"
        ):
        ...
        try:
            ...
            result = {
                ...
                "file_id_sha256": digest(entry["id"].encode()),
                "bytes": int(before.get("size", 0)),
            ...
            if request["operation"] == "collect":
                ...
                if not 0 < result["bytes"] <= maximum or before.get(
                    "mimeType", ""
                ...
                if (
                    ...
                    and hashlib.md5(raw).hexdigest() != before["md5Checksum"]
                ):
                ...
                if (folder / "original").read_bytes() != raw:
    ...
    def status(self, folder, alias):
        ...
        if not folder.exists():
            return {"status": "NOT_RECORDED", "execution_absence_established": False}
        if not (folder / "intent.json").exists():
            return {
                "status": "BLOCKED_RECONCILE",
                "next_action": "Preserve incomplete intent; do not retry automatically.",
            }
        intent = json.loads((folder / "intent.json").read_text())
        if intent["alias"] != alias:
            raise ValueError("DRIVE_REQUEST_ID_CONFLICT")
        receipt = folder / "receipt.json"
        if not receipt.exists():
            return {
                "status": "BLOCKED_RECONCILE",
                "next_action": "Preserve partial evidence; inspect before any new attempt.",
            }
        value = json.loads(receipt.read_text())
        if (
            value["status"] == "PRIVATE_ORIGINAL_COLLECTED"
            and digest((folder / "original").read_bytes()) != value["sha256"]
        ):
            raise ValueError("PRIVATE_EVIDENCE_CHANGED")
        return value

    def upload_status(self, folder):
        """Recover the original receipt without reading or requiring the upload spool."""
        if folder.is_symlink():
            raise ValueError("DRIVE_EVIDENCE_SYMLINK")
        if not folder.exists():
            return {"status": "NOT_RECORDED", "execution_absence_established": False}
    ...
    def store(self, request_id, alias, peer_uid):
        ...
        if alias != "run-artifacts":
            raise ValueError("DRIVE_STORE_ALIAS")
        spool = Path(self.config["upload_spool"]) / request_id
        files, payloads = read_run_bundle(spool)
        ...
        if folder.exists():
            if not (folder / "intent.json").exists():
                return {
                    "status": "UPLOAD_OUTCOME_UNCERTAIN",
                    "next_action": "Inspect incomplete intent; do not allocate another upload.",
                }
            old = json.loads((folder / "intent.json").read_text())
            if old["binding"] != binding:
                raise ValueError("DRIVE_UPLOAD_REQUEST_CONFLICT")
            if (folder / "receipt.json").exists():
                return json.loads((folder / "receipt.json").read_text())
            return {
                "status": "UPLOAD_OUTCOME_UNCERTAIN",
                "next_action": "Reconcile persisted remote IDs; do not create new IDs or retry automatically.",
            }
        folder.mkdir(mode=0o700)
        ...
        try:
            ...
            if len(set(ids)) != len(files) + 1:
                raise ValueError("UNIQUE_REMOTE_IDS_REQUIRED")
            intent = {
                "binding": binding,
                "requester_uid": peer_uid,
                "folder_id": ids[0],
                "file_ids": dict(zip(sorted(files), ids[1:])),
            }
            ...
            for name in sorted(files):
                data = payloads[name]
                file_id = intent["file_ids"][name]
                self.client.private_folder(ids[0])
                value = self.client.create(file_id, name, ids[0], data)
                if value.get("md5Checksum") != hashlib.md5(data).hexdigest() or int(
                    value.get("size", -1)
                ) != len(data):
                ...
                returned = io.BytesIO()
                self.client.download(file_id, returned, LIMIT)
                if returned.getvalue() != data:
                    raise ValueError("DRIVE_UPLOAD_READBACK_MISMATCH")
                observed[name] = {"sha256": digest(data), "file_id": file_id}
                private_write(folder / ("verified-" + name + ".json"), observed[name])
            receipt = {
                "stage": "drive-evidence-storage",
                "status": "PRIVATE_RUN_STORED",
                "collected_at": datetime.datetime.now(
                    datetime.timezone.utc
                ).isoformat(),
                "implementation_sha256": digest(Path(__file__).read_bytes()),
            ...
            return {
                "status": "UPLOAD_OUTCOME_UNCERTAIN",
                "next_action": "Inspect private intent/failure and reconcile remote IDs; no automatic retry.",
            }


def stage_run_bundle(source, spool_root, request_id):
    """Controller-owned staging; preserve source bytes and set service-readable modes."""
    if not re.fullmatch("[a-z0-9][a-z0-9-]{7,79}", request_id):
        raise ValueError("DRIVE_REQUEST_ID_REQUIRED")
    source, spool_root = Path(source), Path(spool_root)
    if (
        spool_root.is_symlink()
        or spool_root.stat().st_uid != os.getuid()
        or not spool_root.stat().st_mode & 0o2000
    ):
        raise ValueError("CONTROLLER_OWNED_SETGID_SPOOL_REQUIRED")
    files, payloads = read_run_bundle(source)
    destination = spool_root / request_id
    if destination.exists() or destination.is_symlink():
        raise ValueError("EXISTING_STAGING_RECONCILE")
    destination.mkdir(mode=0o2750)
    ...
    return {
        "stage": "drive-bundle-staging",
        "status": "STAGED_PRIVATELY",
        "request_id": request_id,
        "file_count": len(files),
        "inventory_sha256": digest(json.dumps(files, sort_keys=True).encode()),
        "uploaded": False,
...
def serve(config_path, socket_path, credentials):
    ...
    if server.family != socket.AF_UNIX or server.getsockname() != socket_path:
        raise ValueError("DRIVE_SOCKET_MISMATCH")
    with server:
        while True:
...
if __name__ == "__main__":
    ...
    p.add_argument("mode", choices=["serve", "request", "stage"])
    ...
    args = p.parse_args()
    if args.mode == "stage":
        if not args.run_root or not args.request_id:
            p.error("run-root and request-id required")
        print(
            json.dumps(
                stage_run_bundle(
                    args.run_root,
                    "/var/lib/research-system/drive-upload-spool",
                    args.request_id,
                )
            )
        )
        ...
        if not args.config or not args.credentials:
            p.error("protected config and credentials required")
        serve(args.config, args.socket, args.credentials)
    else:
        if not all([args.operation, args.alias, args.request_id]):
            p.error("operation, alias and request-id required")
        ...
        print(
            json.dumps(
                request_broker(
                    args.socket,
                    args.operation,
                    {"alias": args.alias, "request_id": args.request_id},
                )
            )
        )
