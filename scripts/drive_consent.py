...
def callback_fields(uri,expected_state):
    ...
    if values.get('state')!=[expected_state] or 'error' in values or len(values.get('code',[]))!=1:
...
def consent(client_file,destination,port):
    ...
    flow=InstalledAppFlow.from_client_config(client,[SCOPE],autogenerate_code_verifier=True)
    ...
    def app(environ,start):
        ...
        try:ids=callback_fields(uri,state)
        except ValueError:
            start('400 Bad Request',[('Content-Type','text/plain')]);return [b'Consent not accepted; return to the operator terminal.']
        response.append((uri,ids));start('200 OK',[('Content-Type','text/plain')]);return [b'Google selection received. Check the operator terminal for token exchange completion.']
    with make_server('127.0.0.1',port,app,handler_class=Quiet) as server:
        server.timeout=300
        print('Open this private consent URL in your browser (do not paste it into chat):',flush=True)
        print(url,flush=True)
        server.handle_request()
    if len(response)!=1:raise ValueError('CONSENT_NOT_COMPLETED_NO_CREDENTIAL_INSTALLED')
    uri,ids=response[0]
    ...
    private_write(destination/'picked-files.json',{'file_ids':ids,'scope':SCOPE,'installation_status':'PENDING_REGISTRY_AND_SERVICE_REVIEW'})
    print(json.dumps({'status':'CONSENT_SAVED_PRIVATELY','selected_files':len(ids),'installed_service':False}))


if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--production-client-confirmed',action='store_true',help='Operator checked OAuth publishing status is In production; not Testing')
    p.add_argument('--client',required=True);p.add_argument('--private-destination',required=True);p.add_argument('--port',type=int,default=8765)
    args=p.parse_args()
    if not args.production_client_confirmed:p.error('Confirm Production status in Google Auth platform before issuing a persistent grant')
